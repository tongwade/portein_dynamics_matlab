function [sum_contact] = getAtomicContact_3(pdb_a)
...
res2_count = count_internal(1)+count_internal(2)+count_internal(3);
res_matrix = a_contact_1(1:end,1:res2_count);
%sum_row = arrayfun(@(x) sum(res_matrix(x,:)),(1:size(res_matrix,1)).');
sum_row=sum(res_matrix,2);
sum_row(sum_row==0)= '';
res2_minus = count_internal(1)+count_internal(2)+count_internal(3)+count_internal(4) ;
...
for i = 1:length(count_internal)-4
    n = n +count_internal(i);
    res_count = res_count + count_internal(i+1)+count_internal(i+2)+count_internal(i+3);
    res_matrix = a_contact_1(1:end,(n:res_count));
    %sum_row = arrayfun(@(x) sum(res_matrix(x,:)),(1:size(res_matrix,1)).');
    sum_row=sum(res_matrix,2);
    sum_row(sum_row==0)= '';
    
    res_minus = res_minus +count_internal(i+1)+count_internal(i+2)+count_internal(i+3)+count_internal(i+4);
    minus_matrix = a_contact_1(m:res_minus,n:res_count);
    %sum_minus_row = arrayfun(@(x) sum(minus_matrix(x,:)),(1:size(minus_matrix,1)).');
    sum_minus_row=sum(minus_matrix,2);
    sum_minus_row(sum_minus_row==0)= '';
    
    sum_contact(i+2,1) = length(sum_row)-length(sum_minus_row);
    
    res_count = res_count -(count_internal(i+2) + count_internal(i+3));
    res_minus = res_minus -(count_internal(i+2)+count_internal(i+3)+count_internal(i+4));
