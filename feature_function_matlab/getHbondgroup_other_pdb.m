function [h_bond_group] = getHbondgroup_other_pdb(pdbname,chain,index,output_path,input_path)
%index is residue id
cd(output_path)
try
    exitCode = system(['hbplus ' input_path lower(pdbname(1:4)) '_' chain '.pdb cc.new -d 3.5 -A 120 0 0 -e HOH " O " 0 -O']);
    if exitCode ~= 0
...
try
    texttt = textread([output_path '/' lower(pdbname(1:4)) '_' chain '.hb2'],'%s','delimiter','\n','whitespace','');
